...
#define QUADRANT_METHOD

static void convert_sc16_nodc_nopower (void *iq_data,
                                 uint16_t *mag_data,
                                 unsigned nsamples,
                                 struct converter_state *state,
                                 double *out_power)
{
    ...
    unsigned i;
    uint32_t I, Q, mag, thresh;

    MODES_NOTUSED(state);

    for (i = 0; i < nsamples; ++i)
    {
        I = abs ((int16_t) le16toh(*in++));
        Q = abs ((int16_t) le16toh(*in++));

#ifdef  QUADRANT_METHOD
...
static struct {
    ...
    { INPUT_SC16,    0, 0, convert_sc16_nodc_nopower,"SC16, integer path" },
